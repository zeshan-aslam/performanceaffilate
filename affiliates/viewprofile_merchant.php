...
<?php
 ...
 while($row=mysqli_fetch_object($res)){
  ...
  while($rows=mysqli_fetch_object($result))
             ...
             $fetchCategory  = "select cat_name from partners_category where cat_id='$merchant_category'";
             $response  =mysqli_query($con,$fetchCategory);
              $data =mysqli_fetch_array($response);

    $links=GetLinks($con,$pgmid);                                                       //total no of links added
 /******************************************************************************/
?>
...
<div class="row">
	<div class="col-md-6">
		<div class="card">
			<div class="card-header">
				<h4 class="card-title" ><?=$lang_MerchantsProfile?></h4>
			...
			<div class="card-body">
				...
				<div class="card-header">
				<h4 class="card-title" style="display: inline-block;"><?=$lang_ProgramDescription?></h4>
				<?php
				
				$statusnotjoined = 'Not Joined';
					if($status == 'approved'){
						echo '<span style="float:right;" class="btn btn-wd btn-success btn-outline">'.ucwords($status).'</span>';
					}else if($status == 'pending'){
						echo '<span style="float:right;" class="btn btn-wd btn-warning btn-outline">'.ucwords($status).'</span>';
					}else{
						echo '<span style="float:right;" class="btn btn-wd btn-warning btn-danger">'.ucwords($statusnotjoined).'</span>';
					}
				?>
				<p><?=$pgm_des?></p>
				</div>
	...
	<div class="col-md-6">
		<div class="card">
			<div class="card-header">
				<h4 class="card-title"><?=$lang_ProgramDetails ?> </h4>
			</div>
			<div class="card-body">
				<p><?=$merchant_company?><?=$lang_hascreated?>
                  <?=$links?> <?=$lang_links?> <?=$title?>.<?=$lang_thus?> <?=$merchant_company?></p>
				  <!-- <p> <a href="index.php?Act=Getlinks" onclick="return confirm('<?=$status?>')"><b><?=$lang_affiliate_getlinks?></b></a> <?=$lang_fromMerchantSite ?></p> -->
                                  <p> <a href="index.php?Act=Getlinks&pid=<?=$pgmid ?>" onclick="return confirm('<?=$status?>')"><b><?=$lang_affiliate_getlinks?></b></a> <?=$lang_fromMerchantSite ?></p>

				  <p><?=$lang_your?><b>Approved</b> <?=$lang_togetlinks?> <?=$merchant_company?></p>
			</div>
		</div>
	</div>
</div>
