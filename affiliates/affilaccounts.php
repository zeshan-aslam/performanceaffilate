...
$suc_msgInfo = $_REQUEST['suc_msgInfo'];
...
if (isset($err)) {
	...
	while ($row = mysqli_fetch_object($res)) {
		...
		$affilate_profileimage	=	stripslashes($row->affilate_profileimage);
		...
		$secretkey   =   stripslashes($row->affiliate_secretkey);
	}
} //is set  else closing
$j = -12;
for ($i = 0; $i <= 24; $i++) {
...
<div class="row">
	<div class="col-md-6">
		<div class="card stacked-form">
			<div class="card-header">
				...
				<span class="textred"><?= $suc_msgInfo ?></span>
			</div>

			<div class="card-body">
				<form method="post" name="logininfoform" action="affil_accounts_validate.php?mode=update">
					...
					<div class="form-group">
						<input name="logininfoform" type="hidden" value="logininfoform" />
						<button type="submit" value="Edit" name="B12" class="btn btn-primary">Edit</button>
					</div>
				</form>
			</div>
	...
	<div class="col-md-6">
		<div class="card stacked-form">
			<div class="card-header">
				<h4 class="card-title">Affiliates Logo</h4>
				<span class="textred"></span>
			</div>
			<div class="card-body">
				<form id="Ajaxform" class="form-inline">
					<?php if ($affilate_profileimage != '') {
					?>
						<div class="addimg_preview">
							<img src="<?php echo 'uploadedimage/' . $affilate_profileimage; ?>" id="" height="150" width="150">
						</div>
					<?php
					} ?>
					...
					<div class="form-group">
						...
						<input type="file" name="ajax_file" accept="image/png, image/gif, image/jpeg" class="form-control" id="Fileinput" onchange="readURL(this);" id="logo">
					</div>
...
<form method="post" name="reg" action="affil_accounts_validate.php?mode=update">
	...
	<div class="row">
		<div class="col-md-12">
			<div class="card stacked-form">
				...
				<div class="card-body">
					<div class="form-group text-center">
						<a href="<?= $track_site_url . "/signup.php?referer=$AFFILIATEID" ?>"><? echo $track_site_url . "/signup.php?referer=$AFFILIATEID"; ?></a><br>

						You can also send your friends to PoweredWords.com<br>
						<a href="https://poweredwords.com/sale_signup.php?referer=<?php echo $AFFILIATEID; ?>">https://poweredwords.com/sale_signup.php?referer=<?php echo $AFFILIATEID; ?></a>
	...
	$transactionfrom = date("Y-m-d", strtotime('-30 days'));
	$transactionto = date("Y-m-d", strtotime('today'));

	$transactionlink = $track_site_url . "/affiliates/transactionhistory.php?key=$secretkey&from=$transactionfrom&to=$transactionto";
	...
	<div class="row">
		<div class="col-md-12">
			<div class="card stacked-form">
				<div class="card-header">
					<h4 class="card-title"><?= $lang_affiliateTransactionLink ?></h4>
				</div>
				<div class="card-body">
					<div class="form-group text-center">
						<?= $lang_affiliateTransactionHelp ?>
					</div>
				</div>
				<div class="card-body">
					<div class="form-group text-center">
						<a href="<?php echo $transactionlink; ?>"><?php echo $transactionlink; ?></a>
					</div>
				</div>
			</div>
		</div>
