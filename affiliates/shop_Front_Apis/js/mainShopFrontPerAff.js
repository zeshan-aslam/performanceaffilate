...
class mallfront {
  ...
  initialize() {
    ...
    const PerAff_mainApiUrl =
      "https://performanceaffiliate.com/performanceAffiliateClone/refApis/searlcoShopFront.php?MADKAI=" +
      PerAffconToken +
      "&selectedCountry=" +
    ...
    fetch(PerAff_mainApiUrl)
      ...
      .then((PerAff_data) => {
        ...
        console.log("color is:", mf.ser_color);
        ...
        if (
          PerAff_data.partnercountries != null &&
          PerAff_data.partnercountries.length > 0
        ) {
  ...
  encodedId = PerAffconToken;
  decodedID = executeAtobFiveTimes(encodedId);
  handleCountryChange() {
    $("#btn_showmore").show();
    ...
    // Reset currentLimit to its initial value
    currentLimit = 30;

    // Perform any further actions with the selected value

    const PerAff_mainApiUrl =
      "https://performanceaffiliate.com/performanceAffiliateClone/refApis/searlcoShopFront.php";
    ...
    $.ajax({
      ...
      success: function (PerAff_data) {
        consDiv = document.getElementById("PerAff_suggesstion-box");
        ...
        if (PerAff_data != null && PerAff_data.length > 0) {
          ...
          for (
            let x = loadedItems;
            x < endIndex && x < PerAff_data.length;
            x++
          ) {
  ...
  loadMore() {
    ...
    const PerAff_mainApiUrl =
      "https://performanceaffiliate.com/performanceAffiliateClone/refApis/loadMoreTest.php";
    ...
    $.ajax({
      ...
      success: function (PerAff_data) {
        ...
        if (PerAff_data.length === 0) {
          $("#btn_showmore").hide();
          mf.showAlert(
            "Sorry no results found, Please select a different country"
          );
        } else if (PerAff_data.length < 30) {
          $("#btn_showmore").hide();
        } else {
