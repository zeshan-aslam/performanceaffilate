...
if ($_GET['action'] === 'multi_Country') {
    ...
    $countryIds = explode(",", $finalOrgVal);

    $sql    = " Delete from partners_affiliate_countries where affiliateid = $AFFILIATEID ";
    $result    = mysqli_query($con, $sql);

    foreach($countryIds as $countryId)
    {
        $sql    = " Insert Into partners_affiliate_countries (affiliateid, countryid) values ($AFFILIATEID, $countryId) ";
        $result    = mysqli_query($con, $sql);
    }


    header("Location:index.php?Act=powered_words");
    exit;
}
...
if ($_GET['action'] === 'multi_Category') {
    ...
    $categoryIds = explode(",", $finalOrgVal);

    $sql    = " Delete from partners_affiliate_categories where affiliateid = $AFFILIATEID ";
    $result = mysqli_query($con, $sql);
    
    foreach($categoryIds as $categoryId)
    {
        $sql    = " Insert Into partners_affiliate_categories (affiliateid, categoryId) values ($AFFILIATEID, $categoryId) ";
        $result    = mysqli_query($con, $sql);
    }
