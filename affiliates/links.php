...
$header_bg1 = $header_bg2 = $header_bg3 = $header_bg4 = $header_bg5 = $header_bg6 = $header_bg7 = $header_bg8 = "header_bg_white";
	...
	global $con;
	//  $today = date('m/d/Y');
	// $sqlcoupon = mysqli_query($con,"select * from affilate_coupon where valid_to > '$today' order by date DESC");
	// $couponcount  = mysqli_num_rows($sqlcoupon);
	$couponcount=0;
	$date1 = date('d/m/Y');
	$today = explode('/',$date1);
	$sqlcoupon = mysqli_query($con, "select c.*, m.merchant_company from affilate_coupon c left join partners_merchant m on c.merchant_id = m.merchant_id order by date DESC");
	while ($row = mysqli_fetch_array($sqlcoupon)) {
...
<li class="nav-item">
	<a class="nav-link" href="index.php?Act=coupons" >
		...
		<p><?=$lang_Coupon?> (<?=$couponcount?>)</p>
	</a>
</li>
<li class="nav-item">
	<a class="nav-link" href="index.php?Act=powered_words" >
		<i class="nc-icon nc-key-25"></i>
		<p>Powered Words</p>
	</a>
</li>
<li class="nav-item">
	<a class="nav-link" href="index.php?Act=mall_front" >
		<i class="nc-icon nc-key-25"></i>
		<p>Mall Front</p>
	</a>
</li>
<li class="nav-item">
	<a class="nav-link" href="index.php?Act=discountHunter" >
		<i class="nc-icon nc-key-25"></i>
		<p>Discount Hunter</p>
	</a>
</li>
