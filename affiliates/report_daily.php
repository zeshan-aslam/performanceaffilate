	...
	<div class="row">
		<div class="col-md-8">
			<div class="card stacked-form">
					...
					<div class="col-md-8">
						<div class="form-group">
							...
							<select name="programs" onchange="document.Getprogram.submit()" class="selectpicker" data-title="Please Select" data-style="btn-default btn-outline" data-menu-style="dropdown-blue">
								<option value="All" <?php if ($PROGRAMID == 0) {
														echo "selected = 'selected'";
													} ?>><?= $lang_home_all_pgms ?></option>
								<?
				...
				<p class="ml-3"><?= $lang_report ?> <? echo "$d.$m.$y" ?></p>
				<div class="table-responsive border">
					<table class="table table-hover table-striped">
						...
						<tbody>
							...
							<tr>
								<td><?= $lang_affiliate_head_click ?>&nbsp;<img alt="" border="0" height="10" src="../images/click.gif" width="10" /></td>
								<td><?= $nclick ?></td>
								<td><?= $currSymbol ?><?= number_format(getCurrencyValue(date("Y-m-d"), $_SESSION['CURRVALUE'], (float)$click), 2) ?></td>
							</tr>
							<tr>
								<td><?= $lang_affiliate_head_lead ?>&nbsp;<img alt="" border="0" height="10" src="../images/lead.gif" width="10" /></td>
								<td><?= $nlead ?></td>
								<td><?= $currSymbol ?><?= number_format(getCurrencyValue(date("Y-m-d"), $_SESSION['CURRVALUE'], (float)$lead), 2) ?></td>
							</tr>
							<tr>
								<td><?= $lang_affiliate_head_sale ?>&nbsp;<img alt="" border="0" height="10" src="../images/sale.gif" width="10" /></td>
								<td><?= $nsale ?></td>
								<td><?= $currSymbol ?><?= number_format(getCurrencyValue(date("Y-m-d"), $_SESSION['CURRVALUE'], (float)$sale), 2) ?></td>
							</tr>
						</tbody>
					</table>
				</div>
		...
		<div class="col-md-4">
			<div class="card stacked-form">
				<div class="export_link m-3 text-center">
					<a class="btn btn-primary" href="#" onClick="window.open('../print_daily.php?aid=<?= $AFFILIATEID ?>&mode=affiliate&date=<?= $selDate ?>&values=<?= $values ?>&program=<?= $PROGRAMID ?>','new','400,400,scrollbars=1,resizable=1');"><b><?= $lang_print_report_head ?></b></a>&nbsp;&nbsp;&nbsp;&nbsp;<a class="btn btn-primary" href="../csv_daily.php?aid=<?= $AFFILIATEID ?>&mode=affiliate&date=<?= $selDate ?>&values=<?= $values ?>&program=<?= $PROGRAMID ?>"><b><?= $lang_export_csv_head ?></b></a>
				</div>
				<?php
