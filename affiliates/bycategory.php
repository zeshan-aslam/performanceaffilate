				...
				<div class="all_cate_list cus_prog_list">
					<ul>
						...
						while ($row = mysqli_fetch_object($ret)) {
							...
							// $cat[$i]=$row->cat_id;
							$sql = "select * from partners_merchant,partners_program where program_status='active' and merchant_category='" . addslashes($row->cat_id) . "' and merchant_id=program_merchantid";
							$result = mysqli_query($con, $sql);
							$catwise_mer = "(" . mysqli_num_rows($result) . ")";          //finding pgm of specified category
							// $i=$i+1;

						?>
							<li><a href="index.php?Act=catWise&amp;joinstatus=catwise&amp;category_id=<?= $row->cat_id ?>&amp;category_name=<?= $row->cat_name ?>"><?= $row->cat_name ?><?= $catwise_mer ?></a></li>
						<?php
				...
				<div class="new_prog_list cus_prog_list">
					<ul>
						...
						while ($row_newPgms = mysqli_fetch_object($res_newPgms)) {
							$k = $k + 1;
						?>
							<li><a href="index.php?Act=programWise&amp;joinstatus=pgmwise&amp;pgm=<?= $row_newPgms->program_id ?>&amp;pgm_url=<?= $row_newPgms->program_url ?>"><?= $row_newPgms->program_url ?></a></li>
						<?
						}
						while ($k <= 10) {
						?>
			...
			<form name="search" method="post" action="index.php?Act=Programs&amp;joinstatus=search">
				...
				<div class="card-body">
					...
					<div class="search_prog_list cus_prog_list">
						<ul>
							<li><a href="index.php?Act=Programs&amp;joinstatus=All"><?= $lang_aff_total ?>(<?= "$all" ?>)</a></li>
							<li><img alt="" border="0" height="<?= $icon_height ?>" width="<?= $icon_width ?>" src="../images/approved.gif" />&nbsp; <a href="index.php?Act=Programs&amp;joinstatus=approved"><?= $lang_aff_approved ?>(<?= "$approved" ?>)</a></li>
							<li><img alt="" border="0" height="<?= $icon_height ?>" width="<?= $icon_width ?>" src="../images/waiting.gif" /> &nbsp;<a href="index.php?Act=Programs&amp;joinstatus=waiting"><?= $lang_aff_waiting ?>(<?= "$waiting" ?>)</a></li>
							<li><img alt="" border="0" height="<?= $icon_height ?>" width="<?= $icon_width ?>" src="../images/suspend.gif" /> &nbsp;<a href="index.php?Act=Programs&amp;joinstatus=suspend"><?= $lang_aff_blocked ?>(<?= "$suspend" ?>)</a></li>
							<li><img alt="" border="0" height="<?= $icon_height ?>" width="<?= $icon_width ?>" src="../images/notjoined.gif" />
								<a href="index.php?Act=Programs&amp;joinstatus=notjoined"><?= $lang_aff_notjoined ?>(<?= "$notjoin" ?>)</a>
							</li>
						</ul>
					</div>
				</div>
