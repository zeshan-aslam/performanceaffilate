...
// $sqlprogram = "SELECT count(*) as scount FROM partners_program, partners_joinpgm, partners_merchant WHERE program_status LIKE ('active')
//   AND joinpgm_programid=program_id AND program_merchantid=merchant_id AND  joinpgm_status='approved' AND joinpgm_affiliateid='$AFFILIATEID'";

$sqlprogram = "SELECT count(*) as scount FROM partners_program, partners_joinpgm, partners_merchant WHERE program_status LIKE ('active')
				AND merchant_status LIKE ('approved') AND program_merchantid=merchant_id 
				AND joinpgm_programid=program_id AND  joinpgm_status='approved' 
				AND joinpgm_affiliateid='$AFFILIATEID'";  
...
$sqlpgms	= " SELECT * from partners_joinpgm, partners_program where joinpgm_affiliateid = '$AFFILIATEID' 
 ...
 while($rows=mysqli_fetch_array($retpgm1)){
	 ...
	 $sqlpending = "select SUM(transaction_amttobepaid) clicktrans, count(*) as countclick from partners_transaction where transaction_joinpgmid = '$joinid' and DATE(transaction_dateoftransaction) = DATE(NOW()) and transaction_status = 'pending'";
	 $resultpending   = mysqli_query($con,$sqlpending);
	 $transpending = mysqli_fetch_array($resultpending);
	 $totalcountpending += $transpending['clicktrans'];
	 
	 $sqlallpending = "select SUM(transaction_amttobepaid) clicktrans, count(*) as countclick from partners_transaction where transaction_joinpgmid = '$joinid' and transaction_status = 'pending'";
	 
	  $resultallpending   = mysqli_query($con,$sqlallpending);
	 $transallpending = mysqli_fetch_array($resultallpending);
	 $totalallcountpending += $transallpending['clicktrans'];
		...
		<div class="col-lg-4 col-sm-6">
			<div class="card card-stats">
				<div class="card-body ">
                  ...
                  <?=$currSymbol?>
                  <?php
                  $value = getCurrencyValue(date("Y-m-d"), $_SESSION['CURRVALUE'], $_SESSION['AFFILIATEBALANCE']);								
								  
                  echo $mainval= number_format((float)$value, 2, '.', '');  

                  ?>
                          </h4><p>Pending: <?=$currSymbol?><?=number_format(getCurrencyValue(date("Y-m-d"), $_SESSION['CURRVALUE'], $totalallcountpending),2)?></p>
							</div>
						</div>
					</div>
				</div>
					...
					<div class="row">
						...
						<div class="col-7">
							<div class="numbers">
								...
								<p>Pending: <?=$currSymbol?><?=number_format($totalcountpending,2)?></p>
							</div>
						</div>
					</div>
				</div>
