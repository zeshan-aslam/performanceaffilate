...
function addClientname( $namee )
{
	$r1     = $this->w - 80;
	$r2     = $r1 + 68;
	$y1     = 40;
	$this->SetXY( $r1, $y1);
	$this->MultiCell( 60, 4, $namee);
} 
...
function addCompanyName( $compneee )
{
	$r1     = $this->w - 80;
	$r2     = $r1 + 68;
	$y1     = 40;
	$this->SetXY( $r1, $y1);
	$this->MultiCell( 60, 4, $compneee);
}
...
{
	$this->SetFont( "Arial", "", 10);
	$length = $this->GetStringWidth( "Notes : " . $remarque );
	$r1  = 10;
	$r2  = $r1 + $length;
	$y1  = $this->h - 45.5;
	$y2  = $y1+5;
	$this->SetXY( $r1 , $y1 );
	$this->Cell($length,4, "Notes : " . $remarque);
}
...
//                  "qte"     => qte,
//                  "tva"     => code_tva );
function addTVAs( $params ,$amount_getall, $Compnay_name)
{
	$this->SetFont('Arial','',8);
	...
	//$this->SetFont( "Arial", "", 8);
	//$this->SetXY( $re, $y1+5 );
	//$this->Cell(20,20,'Total Amount: '.$amount_getall);
    //$this->Cell(17,160,'Make all checks payable to Company Name Payment is due within 30 days');
	$this->addRemarque( "The Gross Amount has been credited to your account" );
