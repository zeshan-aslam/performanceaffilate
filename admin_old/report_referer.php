   ...
   switch($programs)
   {
      case 'AllPgms':
        ...
        if($saletype==1 or $leadtype==1 or $clicktype==1 or $impr_type==1){
            ...
            $tsql  .= ($impr_type==1) ? "OR T.transaction_type = 'impression' " : "";  ##modified on 01.mar.06 to add impressions
		...
		$impSql = "SELECT *,date_format(imp_date,'%b %d, %Y') As impDate FROM partners_affiliate,partners_impression_daily ";
     ...
     case 'All':
		...
		$impSql = "SELECT *,date_format(imp_date,'%b %d, %Y') As impDate FROM partners_affiliate, partners_impression_daily ";
        $impSql.= " WHERE imp_merchantid = '$MERCHANTID' ";
...
<table border='0' cellpadding="0" cellspacing="0"  class="tablebdr"  width="50%" align="center">
         <tr>
             <td height="19" class="tdhead" colspan="2"  align="center"><b>Statistics For Custom Period</b></td>
         </tr>
         <tr>
        ...
        <tr>
                <td colspan="2" align="center" height="20" class="tdhead"><input type="checkbox" name="impr_type" value="1" class="borderless" <?=($impr_type)?"checked='checked'":""?> /> Impression&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; <input type="checkbox" name="clicktype" value="1" <?=$schk?> class="borderless" <?=($clicktype)?"checked='checked'":""?> /> Click&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<input type="checkbox" name="saletype" value="1"  class="borderless" <?=($saletype)?"checked='checked'":""?> /> Sale &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<input type="checkbox" name="leadtype" value="1"  class="borderless" <?=($leadtype)?"checked='checked'":""?> /> Lead
                </td>
        </tr>
        <tr>
                 <td colspan="2" height="13">&nbsp;</td>
...
if($sub=="View" and empty($msg)){
    ...
    if((mysqli_num_rows($result)) ){
        ...
        ?>
 <p align="right">
	<a href="#" onClick="window.open('../print_referer.php?mid=<?=$MERCHANTID?>&from=<?=$From?>&to=<?=$To?>&program=<?=$programs?>&sale=<?=$saletype?>&lead=<?=$leadtype?>&impr=<?=$impr_type?>&click=<?=$clicktype?>','new','400,400,scrollbars=1,resizable=1');" ><b>Print Report</b></a>&nbsp;&nbsp;&nbsp;&nbsp;<a href="../csv_referer.php?mid=<?=$MERCHANTID?>&from=<?=$From?>&to=<?=$To?>&program=<?=$programs?>&sale=<?=$saletype?>&lead=<?=$leadtype?>&impr=<?=$impr_type?>&click=<?=$clicktype?>"><b>Export as CSV</b></a> 	
 </p>
