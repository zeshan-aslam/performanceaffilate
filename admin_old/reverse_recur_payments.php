...
//End Add on 14-July-2006
  ...
  $sql ="select * from partners_transaction T, partners_joinpgm J, partners_recur R, partners_recurpayments P ".
     ...
     ?>
        ...
        <?
             while($row=mysql_fetch_object($ret))
             {
                 ...
                 //geting records from table
                 if($row->transaction_flag==1) 	   $amount =$row->transaction_admin_amount - $row->recurpayments_subsaleamount;
                 ...
                 ?>
					 ...
					 <? if($userobj->GetAdminUserLink('Manage Reverse Recurring Sales',$adminUserId,4)) {  ?>
					 	<a href="reverse_recur_process.php?affiliateid=<?=$row->joinpgm_affiliateid?>&merchantid=<?=$row->joinpgm_merchantid?>&transid=<?=$row->transaction_id?>&recur_payid=<?=$row->recurpayments_id?>&recur_id=<?=$row->recur_id?>&amount=<?=$amount?>">Pay</a>
					<? } else { ?>Pay<? } ?>
					</td>
    				</tr>
