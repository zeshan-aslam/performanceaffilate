...
$id           = $sec->decode_it($affiliate_id);

//Color
$sql_color = "SELECT * FROM color_picks WHERE affiliate_id = 7";
$res = $db->fetch_all($sql_color);
foreach ($res as $row) {
...
function getCountryOfPromotionName($merchant_id)
{
    ...
    $country_pro = "";
    $data = $db->fetch_all($country_pro_query);
...
function getCategoryName($merchant_id)
{
    ...
    $category = "";
    $data = $db->fetch_all($merchant_cat);
...
<div class="main">
 ...
 AND partners_joinpgm.joinpgm_affiliateid='$id' LIMIT 20";

$details    = $db->fetch_all($SqlQuery);
$dataToSend = array();
...
foreach ($details as $data) {
    ...
    $_contents .= '<div><div class="PerAffpartner-img"><img src="' .'https://performanceaffiliate.com/merchants/uploadedimage/'.$data['merchant_profileimage'].'" alt="" height="65px"></div><br><div><b style="color:'.$name_color.';">' .  $companyname . '</b><button type="button" class="btn btn-outline-chashback title" style="height:78px">' . preg_replace('/[^a-z0-9]/i', ' ', substr($data['program_description'], 0, 96)) . '<i class="fas fa-caret-right"></i></button>';
    $_contents .= '<span style="cursor:pointer; color:'.$name_color.';" onclick="PerAff_s_Brand_Disc(' . "'" .cleanData($data['merchant_profileimage']) . "'" . ','."'". $coupon_details ."'".','."'". $valid_to ."'".','."'". $valid_from ."'".',' . "'" . cleanData($data['program_description']) . "'" . ',' . "'" . $link_url . "'" . ',' . "'" . $btn_color . "'" . ',' . "'" . $name_color . "'" . ')" class="terms">View</span></div></div></div></div>'; 
   }
...
$_contents .='<div class="partner-name pt-35" id="PerAff_elefRectwo">
    ...
    foreach ($details as $data) {
        ...
        $_contents .= '<div><div class="partner-img"><br><img src="' . 'https://performanceaffiliate.com/merchants/uploadedimage/'.$data['merchant_profileimage'].'" alt="" height="65px"></div><br><div><b style="color:'.$name_color.';">' .  $companyname . '</b><button type="button" class="btn btn-outline-chashbackPerAff title" style="height:78px">' . preg_replace('/[^a-z0-9]/i', ' ', substr($data['program_description'], 0, 96)) . '<i class="fas fa-caret-right"></i></button><span style="cursor:pointer; color:'.$name_color.';" onclick="PerAff_s_Brand(' . "'" . $data['merchant_profileimage'] . "'" . ',' . "'" . $data['merchant_company'] . "'" . ',' . "'" . $data['program_description'] . "'" . ',' . "'" . $link_url . "'" . ',' . "'" . $btn_color . "'" . ',' . "'" . $name_color . "'" . ')" class="terms">View</span></div></div></div></div>';
    }
