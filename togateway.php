<?
 ...
 //  Header("Location:paymentsuccess.php?type=$type&curid=$curid&amount=$amount&secid=$secid&secpass=$secpass");
 //  exit;
   ...
   if($payment_method=="WireTransfer" || $payment_method=="CheckByMail") {
        $successurl =$track_site_url."/newpay_success.php?paytype=$payment_method&curid=$curid&amount=$amount&secid=$secid&secpass=$secpass";
  ...
  if($payment_method=="WorldPay")
   {
       ...
       $mydomain   = "http://".$_SERVER['SERVER_NAME'];
       $successurl = $track_site_url."/return_world.php?type=$type&curid=$curid&amount=$amount&secid=$secid&secpass=$secpass";
       $failurl    = $track_site_url."/paymentfail.php?curid=$curid";
