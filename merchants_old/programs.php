...
function SaveProgramDetails($programDetails, $MERCHANTID, $currValue, $default_currency_caption, $programId='')
{
	$con = $GLOBALS["con"];
		...
		if($programId) {
			$sql = "UPDATE partners_program SET program_status = 'inactive', ".$sql_common." WHERE program_id = '$programId' ";
		} else {
			$sql = "INSERT INTO partners_program SET 
				program_date = '".date("Y-m-d")."', program_status = 'inactive', ".$sql_common;
		}
		$res = mysqli_query($con,$sql);  
