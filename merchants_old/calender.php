    ...
    */
    function getMonthHTML($m, $y, $showYear = 1)
    {
        ...
        $year = $a[1];
            ...
            $s .="\n";
            $s .="<td width=\"30%\" height=\"100%\" valign='top'>\n";

            $s .= "<table width=\"96%\"  cellspacing=\"1\"  align=\"center\" class=\"tablebdr\" >\n";
            $s .= "<tr>\n";
            $s .= "<td align=\"center\" class=\"tdhead\" colspan=\"7\"><b>Calender</b></td>\n";
            $s .= "</tr>\n";
            $s .= "<tr>\n";
            $s .= "<td align=\"center\" valign=\"top\" class=\"CalHead\" >" . (($prevMonth == "") ? "&nbsp;" : "<a    href=\"$prevMonth\">&laquo;</a>")  . "</td>\n";
            $s .= "<td align=\"center\" valign=\"top\" class=\"calendarHeader\" colspan=\"5\">$header</td>\n";
            $s .= "<td align=\"center\" valign=\"top\" class=\"\" >" . (($nextMonth == "") ? "&nbsp;" : "<a href=\"$nextMonth\">&raquo;</a>")  . "</td>\n";
            $s .= "</tr>\n";
        ...
        $today = getdate(time());
            ...
            while ($d <= $daysInMonth)
            {
                ...
                for ($i = 0; $i < 7; $i++)
                {
                    $class = ($year == $today["year"] && $month == $today["mon"] && $d == $today["mday"]) ? "tdhead" : "tdwbdr";
                    $s .= "<td class=\"$class\" align=\"right\" valign=\"middle\">";
                    if ($d > 0 && $d <= $daysInMonth)
                    {
                       	// $link = $this->getDateLink($d, $month, $year);
					   	if(strlen($d)==1) $dateValue = "0".$d; else $dateValue = $d; 
					   	if(strlen($month)==1) $monthValue = "0".$month; else $monthValue = $month; 
                    	$s .= (($d == "") ? $d : "<a href=\"index.php?Act=daily&amp;d=$dateValue&amp;m=$monthValue&amp;y=$year&amp;month=$month&amp;year=$year\">$d</a>");
                    }
                    else
                    {
                        $s .= "&nbsp;";
