<?
      ...
      //  Header("Location:upgarde_success.php?id=$id&amount=$amount&secid=$secid&secpass=$secpass");
      //  exit;

    if($payment_method=="WireTransfer" || $payment_method=="CheckByMail") {
        $successurl =$track_site_url."/merchants/upgrade_manual.php?paytype=$payment_method&curid=$curid&amount=$amount&secid=$secid&secpass=$secpass";
	...
	if($payment_method=="Paypal")
    {
     ...
     //********************************************************** PAY PAL FORM ***************************************//
	    ...
	    $successurl =	$track_site_url."/merchants/upgarde_success.php?id=$id&amount=$amount&secid=$secid&secpass=$secpass";
	    $failurl    =	$track_site_url."/merchants/index.php?Act=accounts&msg1=Payment Failed Please Try Again";
...
?>
   	<input type="hidden" name="cmd" value="_xclick">
    <input type="hidden" name="business" value="<?=$paypalid?>">
	<input type="hidden" name="item_name" value="Upgradation  Fee This Amount Will Be added to Your System Account">
    <input type="hidden" name="amount" value="<?=$grandtotal?>">
    <input type="hidden" name="currency_code" value="USD">
...
<?
     ...
     if($payment_method=="WorldPay")
   {
       ...
       $mydomain   = "http://".$_SERVER['SERVER_NAME'];
        ...
        $successurl =	$track_site_url."/merchants/returnUpgrade_world.php?id=$id&amount=$amount&secid=$secid&secpass=$secpass";
	    $failurl    =	$track_site_url."/merchants/index.php?Act=accounts&msg1=Payment Failed Please Try Again";
